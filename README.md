...
macOS 和 Android 设置页都提供“Grok 4.1 风格（实验）”预设。它只改变表达方式，不改变白名单、
...
| **Grok 风格预设** | macOS、Android 可选直接、坦率、适度幽默的 Grok 4.1 风格（实验） |
| **会话语气画像** | Mac 本地提取并可脱敏导入 Android；按当前消息只检索少量相关历史样例 |
...
“启动”按规则服务、自动回复服务的顺序补齐未运行项，不会重启已经运行的服务；若 macOS 把服务
标记为已禁用，App 会自动重新启用并注册；“停止”按相反
...
**回复标识。** 可选追加尾注；Android 新配置默认留空，不会强制暴露自动化标记。
