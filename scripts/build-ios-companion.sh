...
rm -rf "$BUILD_DIR/archive" "$BUILD_DIR/export" "$BUILD_DIR/simulator" \
  ...
  "$DIST_DIR/TraceMemoRemote-simulator.app"
...
PLIST
  ...
  if [[ -d "$ARCHIVE_PATH" ]]; then
    cp -R "$ARCHIVE_PATH" "$DIST_DIR/TraceMemoRemote-unsigned.xcarchive"
    echo "真机归档失败，已保留未签名归档：$DIST_DIR/TraceMemoRemote-unsigned.xcarchive"
  else
    echo "未生成真机归档。详细原因：$BUILD_DIR/archive.log"
  fi
fi
...
if [[ -f "$DIST_DIR/TraceMemoRemote.ipa" ]]; then
  ...
  echo "仅生成模拟器包；真机安装需要 Apple Team 与 provisioning profile。"
fi
echo "产物目录：$DIST_DIR"
