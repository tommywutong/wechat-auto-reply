#!/usr/bin/env bash
# 安装 TraceMemo -> DeepSeek -> 微信界面自动回复服务。
# 真实发送只对 core/config.yaml 白名单生效；群聊必须 @ 当前昵称。
set -euo pipefail

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LABEL="com.wxauto.tracememo-autoreply"
...
<dict>
  ...
  <array>
    <string>${REPO_DIR}/scripts/run-tracememo-autoreply.sh</string>
  </array>
  ...
  <string>${REPO_DIR}/var/tracememo-autoreply.log</string>
  <key>StandardErrorPath</key>
  <string>${REPO_DIR}/var/tracememo-autoreply.err.log</string>
</dict>
</plist>
EOF

chmod +x "$REPO_DIR/scripts/run-tracememo-autoreply.sh"
